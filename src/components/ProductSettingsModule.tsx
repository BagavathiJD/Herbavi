...
export default function ProductSettingsModule({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      {/* 1. Left widget panel: Schema definition & Add unit Form card */}
      <div className="space-y-6">
        {/* SQL Schema Definition Explainer */}
        <div className="bg-[#0f172a] text-slate-200 p-5 rounded-2xl border border-[#1e293b] shadow-md flex flex-col justify-between text-left">
          <div className="space-y-2">
            ...
            <p className="text-xs text-slate-350 text-slate-300">
              The products table references the measurements table via a <code className="text-pink-400 font-bold bg-slate-950 p-1 py-0.5 rounded text-[10px]">FOREIGN KEY (measurement_id)</code> relationship:
            </p>
        ...
        <div className="bg-[#0f172a] p-5 rounded-2xl border border-slate-850/80 shadow-md space-y-4">
          ...
          <form onSubmit={handleAddSubmit} className="space-y-4">
            ...
            >
              ...
              <span>{isProcessing ? "Processing statement..." : "INSERT INTO measurements;"}</span>
            </button>
          </form>
        </div>
      </div>
      ...
      <div className="lg:col-span-2 bg-[#0f172a] rounded-2xl border border-slate-850/80 shadow-md overflow-hidden flex flex-col h-full justify-between">
                ...
                {measurements.map((m) => {
                  ...
                  return (
                    <tr key={m.id} className="hover:bg-[#020617]/50 transition-colors">
                      {/* Measurement ID index */}
                      <td className="py-4 px-6 font-mono font-bold text-white">{m.id}</td>
        ...
        {/* Database Status Footer summary */}
        <div className="p-4 bg-gray-50 border-t border-gray-100 text-xs text-gray-505 text-gray-500 font-mono text-left flex justify-between items-center">
          <span>Row Count: {measurements.length} instances active</span>
